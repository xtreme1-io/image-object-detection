...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, auto_size=64):
        ...
        if '*' in p:
            ...
            files = []
            files += glob.glob(str(Path(p) / '**' / '*.*'), recursive=True)
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            # Read image
            self.count += 1
            img0 = cv2.imread(path)  # BGR (COCO、纽迈侧视镜的jpg图像都是BGR)
...
#             return [img2label_path.replace(x.split('.')[-1], 'txt') for x in img_paths]
            return [x.replace(sa, sb, 1)[:x.replace(sa, sb, 1).rfind(u".")+1] + u"txt" for x in img_paths]

        try:
            f = []  # image files
            for p in path if isinstance(path, list) else [path]:#通过配置文件，指定的path是一个（存放图片的）文件夹
        ...
        #self.img_files保存了来自各个层级下的所有图片的文件路径
